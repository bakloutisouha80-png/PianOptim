...
class BiorbdModelExplicitHolonomic(HolonomicBiorbdModel):
    """
    ...
    def compute_v_from_u_explicit(self, u: MX):
        """
        ...
        """
        index_marker_findex = marker_index(self.model, "contact_finger")

        v = MX.sym("v", self.nb_dependent_joints)
        q = self.state_from_partition(u, v)

        # Perform the forward kinematics
        finger_marker_in_g = self.marker(index_marker_findex, None)(q, self.parameters)

        return finger_marker_in_g[2]


    @staticmethod
    def holonomic_torque_driven(ocp, nlp, mapping, numerical_data_timeseries: dict[str, np.ndarray] = None):
        """
    ...
    #     """
    #
    #     q_v = self.compute_v_from_u_explicit(q_u)
