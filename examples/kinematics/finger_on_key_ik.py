...
import numpy as np
from casadi import MX, nlpsol, sumsqr, norm_2, vertcat

model_path = "../../pianoptim/models/pianist_and_key.bioMod"
model = HolonomicPianist(model_path)
# q = model.q_hand_on_keyboard[:, None]

q = MX.sym("q", model.nb_q, 1)

target = model.marker(model.marker_names.index("Key1_Top"), None)(q, model.parameters) + np.array([0, 0.06, 0.06])
target[0] = -0.18516
...
# shoulder_to_pelvis = norm_2(H_arm[0:3, 3] - H_pelvis[0:3, 3])
# f += (shoulder_to_pelvis - 0.5) ** 2
f = 0
...
g = vertcat(g, target[1] - mcp[1])
# idx_arm = segment_names.index("RightUpperArm")
# idx_thorax = segment_names.index("Nipple")
# shoulder_in_thorax = model.homogeneous_matrices_in_global(idx_arm, segment_index=idx_thorax)(q, model.parameters)
# shoulder_in_thorax = shoulder_in_thorax[0:3, 3]
# elbow_in_thorax = model.homogeneous_matrices_in_global(segment_names.index("RightForeArm"), segment_index=idx_thorax)(
#     q, model.parameters
# )
...
for i in [6, 7, 8, 9, 10]:
    f += (q[i] - q0[i]) ** 2
# f += (q[8] + 105 * np.pi / 180) ** 2
# # f += (shoulder_to_pelvis - 0.5) ** 2
# f += 0.1 * (q[5] + 30 * np.pi / 180) ** 2
# f += q[6] ** 2 + q[7] ** 2
# # f += 100 * (q[0] ** 2 + q[1] ** 2 + q[2] ** 2 + q[3] ** 2 + q[4] ** 2)
# f += 10 * (q[10] ** 2 + q[11] ** 2)
